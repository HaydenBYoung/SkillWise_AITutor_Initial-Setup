  ...
  useEffect(() => {
    const initializeAuth = async () => {
      ...
      if (token) {
        ...
        // No access token available: try to silently refresh using httpOnly cookie
        try {
          const refreshRes = await apiService.auth.refresh();
          const accessToken = refreshRes.data?.accessToken;
          if (accessToken) {
            setAccessToken(accessToken);
            // Fetch profile with renewed token
            const response = await apiService.user.getProfile();
            dispatch({
              type: AUTH_ACTIONS.LOGIN_SUCCESS,
              payload: { user: response.data },
          ...
          // no-op; fall through to set loading false
          console.info('Silent refresh failed (not authenticated)');
        }
