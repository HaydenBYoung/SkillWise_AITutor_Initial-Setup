...
```sql
-- NOTE: the project's current SQL migrations use integer SERIAL ids for core
-- tables (users, goals, challenges). The runtime schema uses an integer
-- `goal_id` on `challenges` referencing `goals(id)`.
...
CREATE INDEX idx_challenges_status ON challenges(is_active);
CREATE INDEX idx_challenges_order_index ON challenges(goal_id, created_at);
...
```sql
INSERT INTO goals (user_id, title, description, category, difficulty, target_completion_date) VALUES
