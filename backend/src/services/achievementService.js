...
const achievementService = {
  async getAllAchievements() {
    try {
      const { rows } = await db.query(
        'SELECT id, key, title, description, points, created_at FROM achievements ORDER BY created_at DESC'
      );
      return rows;
    } catch (err) {
  ...
  async getAchievementById(id) {
    try {
      const { rows } = await db.query(
        'SELECT id, key, title, description, points, created_at FROM achievements WHERE id = $1',
        [id]
      );
      return rows[0];
    } catch (err) {
